...
<body class="bg-gray-100">
    <main class="w-11/12 mx-auto mt-4">
        <form action="/super/admin/edit/hospital/{{ Crypt::encrypt($hospital->id) }}" class="w-8/12 mb-6 grid grid-cols-2 mx-auto gap-x-5 px-8 py-5 rounded-md shadow-md mt-8 bg-white" method="post" enctype="multipart/form-data">
            @csrf
            <h1 class="col-span-2 text-2xl font-semibold text-green-600 mb-3">Update Hospital</h1>
            <div class="my-2">
            ...
            <div class="my-2">
                ...
                <input type="text" value="{{ $hospital->email_address }}" name="email_address" placeholder="Enter Hospital's Email Address" class="capitalize border-l-4 border-green-500 w-full rounded-md p-3 shadow-md outline-none block">
                @error('email_address')
                    <p class="text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <button type="submit" class="w-32 py-3 block mx-auto text-center text-white bg-green-600 rounded-md mt-2 shadow-md col-span-2">Update</button>
        </form>
    </main>
</body>
</html>
