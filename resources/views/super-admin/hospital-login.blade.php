...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased bg-gray-100">
        <form action="/loging" class="w-11/12 md:w-4/12 mx-auto bg-white shadow-md rounded-md pb-6 mt-14">
            <h1 class="w-full bg-green-600 text-white py-3 text-center rounded-tr-md rounded-tl-md text-xl mb-4 font-semibold">Hospital Login</h1>
            <div class="px-7">
                @if (session('status'))
                    <p class="text-red-400 text-center my-3">{{ session('status') }}</p>
                @endif
