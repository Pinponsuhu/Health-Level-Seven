...
<body class="w-screen bg-gray-100 h-screen p-4 md:p-8">
    <div class="w-11/12 md:w-10/12 overflow-y-scroll h-full mx-auto shadow p-8 rounded-md bg-white text-green-500">
        ...
        <h1 class="text-xl font-bold mb-4 text-green-500">Change "{{ $details->username }}'s" Passport</h1>
        <form action="/super/admin/change/passport/{{ $id }}" class="md:w-6/12" enctype="multipart/form-data" method="POST">
            @csrf
