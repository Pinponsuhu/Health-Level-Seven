...
@section('content')
    <main class="w-full">
        ...
        <div class="mt-3 px-4 md:px-8">
            <div class="bg-white p-3 md:p-6 rounded-md shadow-md w-full">
                <section class="mt-4 px-4">
                    <h1 class="mt-4 mb-2 text-center text-xl font-semibold text-green-500">Search Result for: {{ $search }}</h1>
                    <form action="/department/bed/search" class="w-11/12 md:w-8/12 mx-auto grid capitalize grid-cols-4 gap-x-3 items-center my-3" method="post">
                        @csrf
