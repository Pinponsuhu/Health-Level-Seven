...
@section('content')
    <main class="w-full">
        ...
        <section class="mt-4 px-8">
            <a href="/department/bed/management" class="block w-28 text-lg text-center ml-6 rounded-full py-3 shadow-md bg-green-500 text-white"><i class="fa fa-arrow-left text-sm mr-1"></i> Back</a>
            <h1 class="mt-4 mb-2 text-center text-xl font-semibold text-green-500">Search Result for: {{ $search }}</h1>
            <form action="/department/bed/search" class="w-8/12 mx-auto grid capitalize grid-cols-4 gap-x-3 items-center my-3" method="post">
                @csrf
